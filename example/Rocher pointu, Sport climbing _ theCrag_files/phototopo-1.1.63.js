...
PhotoTopo.prototype = {
            ...
            if (type == "area" || type == "annotation"){
                ...
                if (pointsA[0] != ""){
                    pointsM = pointsA.map(function convertPoint2Object(p){
                        ...
                        var bits = p.split(/\s+/);
                        var type = bits[2] || '';
                        var visible = (bits[bits.length-1] != 'hidden');
    ...
    /*
     * Remove a topo and clean up it's data bindings
     */
    remove: function remove(){

        var $topo = this.$element;
        $topo.unbind('hover');
        $topo.find('.canvas').removeClass('autozoom');
        $topo.find('svg').remove(); // remove rendered topo svg
        $.removeData($topo[0], 'phototopo'); // remove topo internal data

        var topo = this;
        // clear page topo cache
        var i = PhotoTopo.prototype.shared.topos.indexOf(this);
        if (i != -1) {
            PhotoTopo.prototype.shared.topos.splice(i, 1);
        }
    },
    ...
    calculate: function calculate(model){
        ...
        var cache = {
            ...
            areas:       $.extend(true,{}, model.areas, model.annotations )
        };
    ...
    defaults: {
        ...
        zoomOrigY: 0, //
        autoZoom: false, // If true, then it adds autozoom handle event on bg click
        editable: false,
        zindex: {
            hover: 101,
            select: 100
...
//        }
        ...
        if (window.navigator.msPointerEnabled) {
            document.addEventListener('MSPointerMove', this.scrubMove, {capture: true, passive:false});
            document.addEventListener('MSPointerUp',   this.scrubEnd,  {capture: true, passive:false});
        } else {
            document.addEventListener('touchmove',     this.scrubMove, {capture: true, passive:false});
            document.addEventListener('touchend',      this.scrubEnd,  {capture: true, passive:false});
            document.addEventListener('touchcancel',   this.scrubEnd,  {capture: true, passive:false});
        }
    ...
    pointTypes: {
        ...
        'belayfixed': {
            ...
            group: '2.Belay',
            order: 4,
            svg: '<g id="belayfixed" fill="#fff" stroke="#007" stroke-width="1">' +
                 '<path d="M-1.5 -3.5V -2l -2 2l 2 2v 1.5H -3l -2 -2l -2 2h -1.5V 2l 2 -2l -2 -2v -1.5H -7l 2 2l 2 -2H -1.5z"/>' +
                 '<path d="M7 -3.5h 1.5V -2l -2 2l 2 2v 1.5H 7l -2 -2l -2 2H 1.5V 2l 2 -2l -2 -2v -1.5H 3l 2 2L 7 -3.5z"/>' +
                 '</g>',
            zindex: 4,
            angle: 'lock'
        },
        'belaynatural': {
            ...
            order: 4,
            svg: 
            '<clipPath id="circle-clip"><circle id="circle1" cx="0" r="4" cy="-3" /><circle id="circle2" r="4" cy="2" cx="3"/><circle id="circle3" r="4" cy="2" cx="-3" /></clipPath><g id="belaynatural" fill="none" stroke="#007" stroke-width="1"><rect y="-8" x="-9" height="16" width="18" stroke="none" fill="#fff" clip-path="url(#circle-clip)"/><circle cx="0" r="4" cy="-3" /><circle r="4" cy="2" cx="3"/><circle r="4" cy="2" cx="-3" /></g>',
            zindex: 4,
            angle: 'lock'
        },
        'belaysemi': {
    ...
    render: function render(refreshEvents) {
        ...
        if (!this.$svg){
            ...
            var topo = this;
            this.$element.find('.canvas').toggleClass('autozoom', topo.options.autoZoom);
            if (topo.options.autoZoom) {
        ...
        var pointTypes = this.pointTypes;
        ...
        function bindEvents(el, state, nid){
            var $el = $(el);
            if (!state.type) state.type = 'route';
            ...
            $el.click    (function(e){
                $(document).trigger('topotoggle', {
                    ...
                    node: state,
                    topo: topo,
                    orig: topo.cache[topo.typeMap[state.type]+'s'][state.id].orig,
                    editing: topo.options.editable
                });
            });
...
//            $labels.append(shadow);
    ...
    hover: function hover(nodeId) {
        ...
        if ($('#n'+nodeId).length == 1) {
            this.$element.addClass('cursor-anchor');
        } else {
    ...
    unhover: function unhover(nodeId) {
        ...
        this.$element.removeClass('cursor-anchor')
        if(!this.hasClass(nodeId, 'hover')) return;
    ...
    /*
     * Returns a list of the points in use, by group, with counts
     */
    pointsInUse: function () {
        var routes = this.model.routes;
        var typeStats = [];
        for (var id in routes) {
            var p = routes[id].points;
            for (var c=0; c<p.length; c++) {
                var types = p[c].type.split(" ");
                for (var t=0; t<types.length; t++) {
                    var type = types[t];
                    if (!type || type == '') continue;
                    var pointType = this.pointTypes[type];
                    if (!pointType || !pointType.group) continue;
                    if (typeStats[type]) {
                        typeStats[type].c++;
                    } else {
                        typeStats[type] = {
        ...
        var types = [];
        for (var id in typeStats) {
            if (id == 'label') continue;
            types.push(typeStats[id]);
        }
    ...
    evtStart: function (el, func){
        if (window.navigator.msPointerEnabled) {
            el.addEventListener('MSPointerDown', func, {capture: false, passive:false} );
        } else {
            el.addEventListener('touchstart',    func, {capture: false, passive:false});
            el.addEventListener('mousedown',     func, {capture: false, passive:false});
        }
    },
    /*
    ...
    autoZoom: function(e){
        ...
        var scale = (!this.zoomScale || this.zoomScale == 1) ? 3 : 1;
        var margin = 20;
...
(function ($, window, document, undefined) {
    ...
    $.fn.phototopo = function(options, args) {
        ...
        if (topo){
            ...
            if (options === "remove"){ return topo.remove(args); }
        }
